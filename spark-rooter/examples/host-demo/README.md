...
3. **接自己的身份**：实现 `RunContextPropagator` 把登录态带到 spark 工作线程（`DemoContextPropagator`）；**生产必须**实现 `SessionIdResolver` 绑定登录态（本示例故意用默认实现，启动会 WARN）。
...
# 日志：spark-rooter: 13 tools registered from 5 beans；selfcheck 9 项 OK；SessionIdResolver demo WARN
```
