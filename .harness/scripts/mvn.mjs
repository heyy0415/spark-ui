#!/usr/bin/env node
/**
 * node .harness/scripts/mvn.mjs <args...>   （cwd 固定为 backed/，无需 -f）
 *
 * 统一 Maven 入口：优先 backed/mvnw，其次 PATH 上的 mvn。
 * 强制 JAVA_HOME 指向 JDK 21（backend-standard.md §1），本机 jenv 下存在 openjdk64-21。
 * backed/pom.xml 不存在时直接通过（骨架未初始化，不阻塞前端 CI）。
 */
...
if (!existsSync(join(backed, 'pom.xml'))) {
  console.log('✓ mvn: backed/pom.xml not present, skipping backend build');
  process.exit(0);
}

const args = process.argv.slice(2);
const mvnw = join(backed, 'mvnw');
...
const r = spawnSync(cmd, args, { stdio: 'inherit', env, cwd: backed });
