...
- 阶段 8 前：用户提供真实 OpenAI 兼容端点后首次接真模型，两处失败：(1) Spring AI 默认拼 `/v1/chat/completions`，网关 baseUrl 已含 `/api/v1` → 404；(2) 模型对「帮我把这个订单退款」只规划两步只读工具，`refund.create` 被省略，Run 直接 COMPLETED 而无确认屏。修复：`completionsPath` 按 baseUrl 是否以 `/v{n}` 结尾自适应；Prompt 明确「用户要执行操作时必须包含有副作用步骤，系统会负责确认」。教训：契约 / 自检 / 规则规划器全绿不等于真模型路径可用，真模型路径必须至少跑一遍 e2e；e2e 脚本要能感知 LLM 模式（超时、自检期望）。密钥只经环境变量传入，全树 grep 确认未落盘。
