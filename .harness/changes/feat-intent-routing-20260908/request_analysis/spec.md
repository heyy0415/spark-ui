...
> v2.1 — 吸收 `review/spec_review_v2.md`（APPROVED）6 条 SHOULD：S-1 Awaiting 重 claim 改 deadline 循环；S-2 release 只在未 complete 时调用且对已 complete 的 key 为 no-op；S-3 Registry `findAll()` + `domains(principal)` 前移到 T01；S-4/S-5 用例 ⑥ 期望改精确序列且仅规则模式；非目标补「不新增 order 屏」；S-6 e2e selfcheck 按名字追加条目、④ 放在 §6.2.12 之后。
...
- **安全边界（agent-safety §2 更新）**：分类结果只决定「去 Registry 查哪个领域的候选」；Registry 仍按 tenant / permission 过滤，Gateway 仍逐次鉴权。模型不能发明领域（枚举校验），不能跳过 Registry。`KNOWN_DOMAINS` 来自 Registry：`ToolRegistryClient.domains(principal)` → Registry 侧新增 `ToolRegistryRepository.findAll()`，全部 manifest 经 `DiscoveryPolicy.filter(…, perms)` 后取 `domain` 去重。**带 principal**（agent-safety §2 红线），即「该用户可见的领域集合」；无权领域对该用户直接判 none，不再多一跳 search。
...
- **N6 Gateway 幂等改「先占位后填充」**：`IdempotencyStore` 接口重定义为 `Claim claim(tenantId, key)`（`Claim` 为 sealed：`Owner`（本次拿到执行权）/ `Replay(response)`（已有最终结果）/ `Awaiting(future)`（他人执行中））、`complete(tenantId, key, resp)`、`release(tenantId, key)`、保留 `find(tenantId, key)` 供自检断言终态；**删除** `putIfAbsent`（无外部实现者）。`InMemoryIdempotencyStore` 用 `ConcurrentHashMap<String, CompletableFuture<Response>>`。`InvokeToolUseCase` 第 4 步：`Owner` → 执行，成功 `complete`，任何异常 `finally release`；`Replay` → 直接返回，审计 `status=replayed`（仅日志口径，非契约）；`Awaiting` → 等待至多 `execution.timeoutMs`：正常完成 → 返回并审计 `replayed`；`release` 触发的 `completeExceptionally` → **在同一 deadline 内循环重新 claim**（拿到 `Owner` 则执行，再次 `Awaiting` 则继续等，deadline 到 → `TIMEOUT`）。`release` **只在未 `complete` 的异常路径**调用，且对已 `complete` 的 key 为 no-op（不会把已完成结果删掉导致重放失效）。自检 `GatewayIdempotencySelfCheck` **只测 `IdempotencyStore`**（不经 `InvokeToolUseCase`，避免 handler 表注入问题）：线程 A claim → Owner，`CountDownLatch` 保证线程 B 在 A `complete` 前 claim → Awaiting；A complete 后 B 拿到同一 Response；另一 key：A claim 后 `release`，B 的 Awaiting 收到异常后重新 claim → Owner；第三条：A complete 后再 `release` 为 no-op，`find` 仍返回结果。日志 `selfcheck: gateway idempotency claim OK`；`SelfCheckRunner` 计数 4 → 5，`deploy-verify.sh` 的期望 4 同步改 5。
...
- `e2e-backend.sh` 新增用例（用例 ①②④⑤⑥ 两种模式都跑；③ 仅 LIVE）：
  ...
  ⑥（仅规则模式；LIVE 下打印 `skipped (live mode)`，因模型可能选其它工具）「订单」+ 无 `pageContext` → 事件恰为 `run.started tool.selected tool.started tool.completed run.completed`，`tool.selected.toolId == order.list.search`。
  用例顺序：④ 必须放在现有 §6.2.12（令牌重放）之后，避免全文件 `refund.create succeeded` 计数变 2；selfcheck 断言按**名字**追加 `gateway idempotency claim OK` 一条（现脚本是逐名 check，不存在数字 4）。
...
- **不**引入分类结果缓存 / 向量检索。
- **不**为 order 领域新增 UI 屏（`UiSchemaBuilder` 仍只有 refund 确认屏与结果屏）；order 链路 `run.completed` 无 `ui.replace`，前端显示「已完成」，是首期既有行为。
...
- [ ] 用例 ⑥（规则模式）：事件恰为 `run.started tool.selected tool.started tool.completed run.completed`，`toolId == order.list.search`；LIVE 模式跳过。
...
- 分类器与规划器共用同一 `ChatClient` Bean 与三个环境变量。`domains(principal)` 只在模型分类路径调用，规则命中时零额外 Registry 查询。
