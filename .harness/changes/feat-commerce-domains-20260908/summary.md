...
| 2 | 需求评审 | IN PROGRESS | 1/3 | v1 RR（7 MUST：规则规划器排序推演与断言不符、实体抽取在拦截之后、路由顺序未定、spi 签名违红线、重校验语义矛盾、订单数三处不一致、task 超时；12 SHOULD）→ v2 RR（5 MUST：空 Form 违契约、⑮ 复用已退款夹具、10007 不在默认列表、自检数不一致、OrderSnapshot 缺 productName；6 SHOULD）→ v3 RR（3 MUST，均文字补丁：⑫ 断言漏改、夹具表缺金额、售后枚举 / 分布未定；5 SHOULD）→ 按评审「最小补丁清单」吸收为 v3.1。**3 轮上限，等待 HITL ②** | 2026-09-08 |
| 3 | 编码实现 | TODO | — | — | — |
| 4 | 编码评审 | TODO | 0/2 | — | — |
| 5 | 代码推送 | TODO | — | — | — |
| 6 | CI 验证 | TODO | — | — | — |
