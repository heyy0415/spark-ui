...
| 3 | 编码实现 | DONE | — | T01 `c8b9b36`（backed → spark-rooter，mvn verify 0，e2e 规则 109/109）；T02 `2096609`（fronted → spark-ui，契约 $id，fe ci 0）；T03 `405c78b` check-rename；T04 `241ab70` 契约；T05 `bbe8922` spi 注解 / 端口；T06 `0b02df9` registry / gateway 去身份；T07a `e358c1a` runtime sessionId 双绑定；T07b `c41f03b` web-mvc 剥离；T08 `7c8368a` starter 自动装配；T09a `eaef57c` 扫描 / 推导 / 代理调用 / ToolMetaRegistry；T09b `b0acfbe` 四领域 @SparkTool（12 tools from 4 beans，parity 12/12）。T10 `8246356` host-demo（删 app，e2e ⑭ ⑭' ㉖）；T11 `f5d4de5` ArgumentExtractor + 值校验；T12 `5d45502` 记忆 / 序数 / 澄清屏 / Card.actions；T13 `9485f00` 前端去 pageContext + Card.actions + transport 注入；T14 随 T10–T13 落地（e2e 规则 145/145、e2e-frontend 37/37、deploy-verify 12/12）；T15 `27ae150` 文档同步；T16 全链路验收（ci 0 / e2e 145 / deploy-verify 12 / e2e-frontend 37 / doctor 0）→ `coding/coding_report_v1.md` | 2026-09-10 |
| 4 | 编码评审 | DONE | 2/2 | v1：后端 RR（4 MUST / 10 SHOULD）、前端 CR（1 MUST / 4 SHOULD）→ 回修 `dc0ab7f`；v2：双端 APPROVED（后端 2 SHOULD、前端 2 SHOULD 已吸收）| 2026-09-10 |
| 5 | 代码推送 | IN PROGRESS | — | GitHub https://github.com/heyy0415/spark-ui.git | 2026-09-10 |
| 6 | CI 验证 | TODO | — | — | — |
| 7 | 部署验证 | TODO | — | — | — |
| 8 | 用户确认 | TODO | — | — | — |
...
## 待办（跨 change）
- 评审 v2 LOW：`pendingMessage` 在下一 Run 不写记忆时不清；`spark.runtime.run-ttl ≥ token-ttl` 无守卫；`ConfirmationTokenService` 「双绑定」注释应写明 conversationId 取自 Run、实际隔离因子是 sessionId。
