...
| 类型 | feat |
| 状态 | DONE（8 阶段全部完成；主 PR #2 + 修复 PR #3 / #4 已合入或待合） |
...
| 6 | CI 验证 | DONE | — | 本地 `pnpm -C .harness run ci` 9 步 0 → [ci_result/ci_summary.md](ci_result/ci_summary.md)。**GitHub Actions 实证**：run `34753846814`（PR #4，commit `6c46054`）全部步骤 success——这是本仓 CI 首次在 GitHub 上转绿。此前三次红各修一处：shellcheck 版本差（PR #3）、mvnw 指向内网 Nexus、host-demo 离线缺 actuator（均在 PR #4） | 2026-09-13 |
...
| 8 | 用户确认 | DONE | — | 用户「确认交付」。确认时 CI 仍红，我按「CI 绿再收口」暂缓；PR #4 转绿后记 DONE。三个 CI 修复 PR（#3 / #4）的 4 处问题全部记入经验沉淀 | 2026-09-13 |
...
| 8 | CI 首跑红三次，每次一个"本机有、runner 没有"的隐含依赖：① ubuntu 旧版 shellcheck 报 SC2317（本机 0.11 报 SC2329）；② `maven-wrapper.properties` 指向公司内网 Nexus（本机 `~/.m2/settings.xml` 有镜像所以从没暴露，**别人 clone 后第一步就挂**）；③ host-demo 离线打包缺 actuator（对 starter 是 optional，不在传递闭包） | ① `disable` 两个码 + `check-shell` 补漏检的 2 个脚本；② wrapper 改 Maven Central + **doctor 新规则**：10 个构建配置文件禁内网域名，双向自证；③ CI 加 `go-offline` 预热。共同教训：**"本地绿"证明不了"干净环境绿"**，开源项目的门禁必须在无缓存、无内网的 runner 上跑过一次才算数 |
