...
jq -r '.[] | "INSERT INTO `orders` (" + (keys_unsorted | map("`"+.+"`") | join(",")) + ") VALUES (" + ([.[]] | map(if type=="string" then "\x27"+(.|gsub("\x27";"\x27\x27"))+"\x27" elif .==null then "NULL" else tostring end) | join(",")) + ");"' orders.json | mysql -u root -p strato
```
